...
for teta in numpy.arange(0.01, numpy.pi / 2, 0.00001):
    mn1 = abs(
        (numpy.cos(numpy.pi / 2 * numpy.sin(teta))
         / numpy.cos(teta))
    )

    mn2 = abs(
        numpy.sin((N * k * d * (1 - numpy.cos(teta)) / 2))
        /
...
print("Ширина головної пелюстки в площині H = "
      + str(round(SGP1, 2)) + "°")

print("Ширина головної пелюстки в площині E = "
      + str(round(SGP2, 2)) + "°")
...
ax.plot(steps, FC, linewidth=0.7, label="$F_H(θ)$")
...
fig.savefig(
    "ДС_директорної_антени_варіант_24.jpg",
    dpi=600
)
